...
vector<int> nextGreaterElement(vector<int> el){
    int n = el.size();
    vector<int> nge (n,-1);
    
    stack<int> st;
    // we will increase the array size to double -1 to copy the prev element to the end to consider the circular array

    for(int i = 2 *(n-1);i>=0;i--){
         
         // pop the lesser element 
         while(!st.empty() && st.top()<=el[i%n]){
            st.pop();
         }
        ...
        // push the element to stack as it can be 
        st.push(el[i%n]);
    }
  return nge;
}
...
*/
int main(){
vector<int> el ={3,10,4,2,1,2,6,1,7,2,9};
vector<int> nge = nextGreaterElement(el);
for(auto s: nge){
    cout<< s<<endl;
}
}