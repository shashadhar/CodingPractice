#include <iostream>
#include <vector>
#include <algorithm>
#include <queue>
#include <math.h>
#include <unordered_map>
#include <string>
#include <set>
#include <map>

using namespace std;


int maxPlatfrom(vector<float> arr , vector<float> dep,int n){
    int plt = 1;
    int count = 1;
    sort(arr.begin(), arr.end());
    sort(dep.begin(), dep.end());

    int i=1,j=0;
    while (i  < n && j < n)
    {
       if(arr[i] <= dep[j]){
         i++;
         count++;
       }else{
        count--;
        j++;
       }
     plt = max(plt, count);   
    }
return plt;    

}
/*
We are given two arrays that represent the arrival and departure times of trains that stop at the platform. We need to find the minimum number of platforms needed at the railway station so that no train has to wait.

Examples 1:

Input: N=6, 
arr[] = {9:00, 9:45, 9:55, 11:00, 15:00, 18:00} 
dep[] = {9:20, 12:00, 11:30, 11:50, 19:00, 20:00}

Output:3
*/

int main(){
    vector<float> arr = {9.00, 9.45, 9.55, 11.00, 15.00, 18.00} ;
    vector<float> dep ={9.20, 12.00, 11.30, 11.50, 19.00, 20.00};
    cout << "Maximum platfrom:"<< maxPlatfrom(arr,dep,6)<<endl;
}